...
# class ImportanceIntegratedModule(torch.nn.Module):
#     def __init__(self, original_model, importance_scores):
#         super().__init__()
#         self.model = original_model
#         self.importance_scores = importance_scores
        
#     def forward(self, x):
#         # Integrate fixed importance scores into forward pass
#         for i, layer in enumerate(self.model.features):
#             if isinstance(layer, torch.nn.Conv2d):
#                 x = layer(x)
#                 x = x * self.importance_scores[i]
#             else:
#                 x = layer(x)
#         return x
...
class Pruner:
    ...
    def TrainScalingFactors(self, root, num_epochs, fc_lr, conv_lr, momentum):
        for param in self.model.parameters():
            param.requires_grad = False
        ...
        for sf in self.scaling_factors.keys():
            params_to_optimize.append(self.scaling_factors[sf])

        optimizer_alpha = torch.optim.SGD(params_to_optimize, lr=fc_lr, momentum=momentum)
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer_alpha, milestones=[30, 60], gamma=0.1)
    ...
    def RestructureNextLayer(self, layer_index, filter_index):
        ...
        next_new_conv = torch.nn.Conv2d(in_channels=new_in_channels,
                                            out_channels=next_conv_layer.out_channels,
                                            kernel_size=next_conv_layer.kernel_size,
                                            stride=next_conv_layer.stride,
                                            padding=next_conv_layer.padding,
    ...
    def PruneAndRestructure(self, filters_to_prune):
        ...
        for layer_to_prune in filters_to_prune:
            ...
            for filter_to_prune in filters_to_prune[layer_to_prune][::-1]:
                if isinstance(self.model.features[layer_to_prune], torch.nn.Conv2d):
                    self.model.features[layer_to_prune] = self._PruneSingleFilter(layer_to_prune, filter_to_prune)
                if isinstance(self.model.features[next_layer_index], torch.nn.Conv2d):
                    self.model.features[next_layer_index] = self._RestructureNextLayer(layer_to_prune, filter_to_prune)
    ...
    def Finetune(self, num_epochs, learning_rate, momentum, checkpoint_epoch):
        ...
        for epoch in range(num_epochs):
            ...
            for inputs, labels in self.train_loader:
                ...
                for i, layer in enumerate(self.model.features):
                    if isinstance(layer, torch.nn.Conv2d):
                        ...
                        if i in self.importance_scores:
                            outputs = outputs * self.importance_scores[i].cuda()
                    else:
                        outputs = layer(outputs)
            ...
            logger.info(f'Loss: {epoch_loss:.4f}, Train Accuracy: {train_acc:.2f}%, Validation Accuracy: {val_acc:.2f}%')
            
            if val_acc > best_acc:
                best_acc = val_acc
                
            # Check stopping condition
            if best_acc - val_acc > self.tau:
                logger.info(f"Stopping at epoch {epoch} due to accuracy drop")


    def ImportanceAwareFinetuning(self, num_epochs, fc_lr, conv_lr, momentum):
        criterion = torch.nn.CrossEntropyLoss()
        optimizer = torch.optim.SGD(self.model.parameters(), lr=fc_lr, momentum=momentum)
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[30, 60], gamma=0.1)

        for i, layer in enumerate(self.model.features):
            if isinstance(layer, torch.nn.Conv2d):
                layer.weight.data *= self.importance_scores[i].cuda()
                layer.bias.data *= self.importance_scores[i].cuda()
